...
describe('Footer Component', () => {
  ...
  test('Displays the About section with text', () => {
    render(<Footer />);
    expect(
      screen.getByText(/Party-Up is a social network/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /connecting people through shared events and experiences/i,
      ),
    ).toBeInTheDocument();
  });
  ...
  test('Display the copyright information', () => {
    render(<Footer />);
    expect(
      screen.getByText(/© 2024 Party-Up. All rights reserved./i),
    ).toBeInTheDocument();
  });
});
